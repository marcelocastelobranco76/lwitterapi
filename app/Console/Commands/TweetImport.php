...
class TweetImport extends Command
{
     ...
     * @var string
     */
    protected $description = 'Import tweet data and save in lwitterapi_db database';
   ...
   public function __construct()
      ...
      public function handle()
    {
      ...
      /** Setar as chaves de acesso **/ 
        ...
        $conta = '?screen_name='.$account.'&count=50&include_rts=true&geo_enabled=true';
      
        $twitter =  new TwitterAPIExchange($chavesAcesso);
      
        $string = json_decode($twitter->setGetfield($conta)
       ...
       foreach($string as $items)
        {
                ...
                $dataCriacaoTweet =  $items['created_at'];

                $returnedTimestamp = strtotime($dataCriacaoTweet);

                $formataDataTweet = date('Y-m-d',$returnedTimestamp);
                
                $conteudoTweet = $items['text'];

                $localizacaoTweet = $items ['geo']['coordinates']['place']['name'];     

                $likesTweet = $items['favorite_count'];

                $retweetsTweet = $items['retweet_count'];    
            ...
            Tweet::create([
                'criacao' => $formataDataTweet,
                'content' => $conteudoTweet,
                'localizacao' => $localizacaoTweet,
                'likes' => $likesTweet,
                'retweets' => $retweetsTweet,
                'account' => $account
